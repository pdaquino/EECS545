...
and run tests for the evasion strategies. The next stage will be working on firing strategies, using the same approach. We will then repeat these processes while trying to reduce the number of features used. The last milestone is writing the
project report.

\subsection{Training data}
We have now written code that allows us to randomly select an evasion strategy when the robot is fired upon. The next step is 
