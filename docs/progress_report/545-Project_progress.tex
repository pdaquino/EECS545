...
Building a Robocode agent equipped with machine learning algorithms to bette inform its decision making will be broken into two unique tasks: evasion and targeting. Each task will have a subset of strategies which the agent can choose to employ in order to best carry out the current task (evasion or targeting). In order to decide which strategy to employ for a given task, the agent will make use of machine learning in order analyze the current situation within the environment and choose the strategy which will best achieve 
...
In order to determine if the used method was most effective in dodging the bullet that was fired at the agent, we need to devise a way to track a bullet. In Robocode this is not a straight forward task as the agent cannot sense a bullet using its radar. However, in order to fire a bullet a tank needs to use up some of its own energy. The radar-sensor can detect a drop in energy 
